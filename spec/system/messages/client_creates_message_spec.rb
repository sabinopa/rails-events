...
describe 'Client creates message' do
  it 'and sees the field to write an message on order page' do
  ...
  it 'and sees every message sent and reveiced before' do
    ...
    message2 = Message.create!(body: 'Ok. Eles tomam refrigerante?', sender_id: 1, sender_type: 'Supplier', receiver_id: 1, receiver_type: 'Client', order_id: order.id)
    message2 = Message.create!(body: 'Apenas 3 tomam, mas no geral preferimos suco.', sender_id: 1, sender_type: 'Client', receiver_id: 1, receiver_type: 'Supplier', order_id: order.id)
    login_as(client, :scope => :client)
    visit order_path(order.id)

    expect(page).to have_content 'Priscila (Fornecedor):'
